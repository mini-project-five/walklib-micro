...
# set -e  # 오류 시 스크립트 중단 (all 빌드에서 문제 발생으로 비활성화)
...
build_and_deploy_service() {
    ...
    # Docker Compose 서비스 재시작 (비활성화 - all 빌드 시 문제 방지)
...
build_and_deploy_all() {
    ...
    for service in "${JAVA_SERVICES[@]}"; do
        echo -e "${BLUE}📦 $service 빌드/배포 진행 중... ($((SUCCESS_COUNT + FAIL_COUNT + 1))/${#JAVA_SERVICES[@]})${NC}"
        build_and_deploy_service "$service" "$VERSION"
        local result=$?
        if [[ $result -eq 0 ]]; then
            ((SUCCESS_COUNT++))
            echo -e "${GREEN}✅ $service 성공${NC}"
        else
            ((FAIL_COUNT++))
            FAILED_SERVICES+=("$service")
            echo -e "${RED}❌ $service 실패 - 다음 서비스로 계속 진행${NC}"
        fi
        echo -e "${BLUE}🔄 다음 서비스로 진행합니다...${NC}"
        echo ""
    done
    
    # Frontend 빌드 (있는 경우)
    if [[ -d "frontend" ]]; then
        echo -e "${BLUE}📦 frontend 빌드/배포 진행 중...${NC}"
        build_and_deploy_service "frontend" "$VERSION"
        local result=$?
        if [[ $result -eq 0 ]]; then
            ((SUCCESS_COUNT++))
            echo -e "${GREEN}✅ frontend 성공${NC}"
        else
            ((FAIL_COUNT++))
