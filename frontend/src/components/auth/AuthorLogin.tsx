  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignup) {
        ...
        // JWT 기반 작가 회원가입 (userName은 백엔드에서 자동 생성)
        console.log('회원가입 요청 시작:', formData);
        const response = await authApi.signup({
          realName: formData.name,  // 실명
          penName: formData.penName,  // 필명
        ...
        // JWT 기반 작가 로그인
        console.log('로그인 요청 시작:', { email: formData.email });
        ...
        const userData = {
          ...(response as any),
          penName: (response as any).userName || '신진작가',
          userType: 'author',
          works: [],
          totalViews: 0
        };
        
        console.log('로그인 성공, onLogin 호출:', userData);
        onLogin(userData);
      }
    } catch (error) {
      console.error('작가 인증 중 오류:', error);
